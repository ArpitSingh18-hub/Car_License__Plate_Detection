...
# New Configuration: Inference Image Size (smaller = faster)
DEFAULT_INFERENCE_SIZE = 480


# --- Helper Functions (No changes needed, re-using previous functions) ---
...
def draw_boxes_and_confidence(image_np, results, scale_factor=1.0):
    ...
    is_bgr = len(image_np.shape) == 3 and image_np.shape[2] == 3 and image_np.dtype == np.uint8 and image_np[0, 0, 0] != \
             image_np[0, 0, 2]
...
def process_video(uploaded_file, model, confidence, inference_size):
    ...
    if uploaded_file is not None:
        ...
        with col_results:
            ...
            fps_text = st.empty()  # Placeholder for FPS display
        ...
        while cap.isOpened():
            ...
            resized_frame = cv2.resize(frame,
                                       (inference_size, int(original_height * (inference_size / original_width))))

            # Run YOLOv8 detection on the SMALLER frame
            results = model.predict(
                source=resized_frame,
                conf=confidence,
                imgsz=inference_size,  # Use the resized size for optimal model processing
            ...
            if total_frames > 0:
                progress_bar.progress(frame_count / total_frames,
                                      text=f"Processing frames... {frame_count}/{total_frames}")
...
# --- Image Processing Function (Adjusted for Inference Size) ---
...
def process_image(uploaded_file, model, confidence, inference_size):
    ...
    if uploaded_file is not None:
        ...
        resized_image = cv2.resize(image_np_rgb,
                                   (inference_size, int(original_height * (inference_size / original_width))))
        ...
        with col_results:
            ...
            if results and results[0].boxes and len(results[0].boxes) > 0:
                ...
                annotated_image_rgb_np = draw_boxes_and_confidence(image_np_rgb.copy(), results,
                                                                   scale_factor=scale_factor)

                st.image(annotated_image_rgb_np, use_container_width=True, caption="Detected License Plate(s)")

                st.success(
                    f"Detected {len(results[0].boxes)} license plate(s) above {int(confidence * 100)}% confidence.")
